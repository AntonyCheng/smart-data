...
写入 `output/charts/` 的 SVG 必须是**合法的独立 XML 文档**：根 `<svg>` 带 `xmlns` 与 `viewBox`，每个属性都要写成 `name="value"`（不得出现 `tabular-nums` 这类没有取值的裸属性），排版样式用 `style="…"` 或 `<style>`，标签全部闭合。非法 SVG 在预览页会解析中断、显示为碎图。
