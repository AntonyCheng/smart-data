...
- **`auth/`** — JWT (Bearer, sessionStorage on the client), phone-number registration (always `user` role), `AuthSeedService` seeds one admin. `AuthContextMiddleware` binds identity into `AsyncLocalStorage`; every service is tenant/user-scoped via `IdentityService.getIdentity()`.
